...
const ROLE_DASHBOARD: Record<Role, string> = {
  student: "/dashboard",
  industry_supervisor: "/dashboard",
  school_supervisor: "/dashboard",
  admin: "/dashboard",
};
...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const result = login({ emailOrMatric, password, role });
      setLoading(false);
      if (result.success) {
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-accent/50 border border-accent rounded-lg p-3 text-xs text-accent-foreground">
              <strong>Demo:</strong> Use any email/matric + password "password123" for any role. Or leave email blank and just pick a role.
            </div>
